  ...
  return new THREE.ShaderMaterial({
    ...
    fragmentShader: /* glsl */ `
      ...
      void main() {
        ...
        vec3 col = vec3(0.015, 0.033, 0.062);                 // near-black navy paint
        col += vec3(0.10, 0.30, 0.42) * pow(key, 2.0) * 0.85; // cyan/blue key
        col += vec3(0.22, 0.10, 0.34) * pow(rim, 3.0) * 0.55; // purple rim
        col += vec3(0.05, 0.14, 0.22) * up * 0.35;            // sky bounce

        // specular sheen -> metallic read
        vec3 H = normalize(normalize(vec3(0.55, 0.75, 0.6)) + V);
        col += vec3(0.45, 0.85, 1.0) * pow(max(dot(N, H), 0.0), 46.0) * 0.5;
  ...
  return new THREE.ShaderMaterial({
    ...
    fragmentShader: /* glsl */ `
      ...
      void main() {
        ...
        vec3 col = vec3(0.05, 0.20, 0.30) * (0.35 + f * 1.5);
        col += vec3(0.05, 0.18, 0.26) * lines * 0.35;
        col += vec3(0.40, 0.85, 1.0) * band * 0.6;
...
export async function loadHoloCar(): Promise<HoloCar> {
  ...
  const holo = track(bodyMaterial(scanUniform, toCarUniform, 0x22d3ee));
  const holoAccent = track(bodyMaterial(scanUniform, toCarUniform, 0x8b5cf6));
