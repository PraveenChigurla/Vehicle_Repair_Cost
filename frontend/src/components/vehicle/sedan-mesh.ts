...
export function buildPlatform() {
  ...
  const gridMat = track(
    new THREE.LineBasicMaterial({ color: 0x00D9FF, transparent: true, opacity: 0.15 }),
  );
  const grid = new THREE.GridHelper(9, 18, 0x00D9FF, 0x00D9FF);
  ...
  const mat = new THREE.ShaderMaterial({
    ...
    fragmentShader: /* glsl */ `
      ...
      void main() {
        ...
        gl_FragColor = vec4(vec3(0.0, 0.85, 1.0), fade * uOpacity);
      }
    `,
  });
